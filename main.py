...
def post_add(request: Request, content: str = Form(...), status: str = Form(...), db: Session = Depends(get_db)):
    session_key = request.cookies.get("session_key")
    todo = create_todo(db, content=content, status=status, session_key=session_key)
