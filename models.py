...
def create_todo(db: Session, content: str, status: str, session_key: str):
    todo = ToDo(content=content, status=status, session_key=session_key)
    db.add(todo)
    db.commit()
    db.refresh(todo)
    return todo
...
def update_todo(db: Session, item_id: int, content: str, status: str):
...
def get_todos(db: Session, session_key: str, skip: int = 0, limit: int = 10):
